...
class PhysicsBase(PPPY):
    ...
    def __init__(self, dt, method, name, tag, case, grid, prognostic_variables,
        ...
        """
        super().__init__(dt, method, name, tag, inputcase=case.description,
                         grid=grid.filename)
        self.case = case
        self.grid = grid
        ...
        with self.case:
            ...
            for var in ('Zs', 'Ps', 'sshf', 'slhf', 'z0', 'z0h', 'z0q', 'Tskin', 'Ts', 'ustar',
                ...
                if self.case.contains(var):
                    ...
                    logging.warning('%s not found in the netCDF driver!', var)
    ...
    def forcing(self, state, timestep, timestep_number):
        ...
        with self.case:
            ...
            # Surface
            if 'Ts' in state:
                interp = self.case.get_interpolator('ts_forc', self.grid)
                state['Ts'] = interp([time])[0]
            if 'Tskin' in state:
                interp = self.case.get_interpolator('tskin', self.grid)
                state['Tskin'] = interp([time])[0]
            if 'Ps' in state:
                interp = self.case.get_interpolator('ps_forc', self.grid)
            ...
            if 'z0' in state:
                interp = self.case.get_interpolator('z0', self.grid)
                state['z0'] = interp([time])[0]
            if 'z0h' in state:
                interp = self.case.get_interpolator('z0h', self.grid)
                state['z0h'] = interp([time])[0]
            if 'z0q' in state:
                interp = self.case.get_interpolator('z0q', self.grid)
